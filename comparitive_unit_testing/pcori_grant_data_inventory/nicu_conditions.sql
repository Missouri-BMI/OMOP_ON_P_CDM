...
-- Conditions during or before the NICU admission
cond as
(select coh.person_id, cond.col1 as condition_concept_id, c.concept_name as condition_concept_name,
max(case when co.condition_concept_id is null then 0 else 1 end) as condition_present
from cohort coh
join (values 321689, --Apnea
4169095, --Bradycardia
44784217, --Cardiac arrhythmia
316139, --Heart failure
321588, --Heart disease
319138, --Respiratory condition of fetus OR newborn
313023, --Chronic respiratory disease in perinatal period
312723, --Congenital heart disease
436519, --Perinatal intraventricular hemorrhage
4308227, --Neonatal necrotizing enterocolitis 
600520, --Fetal disorder due to and following operative procedure on mother 
43530894, --Fetal disorder due to disease in mother 
...
join concept_ancestor ca on cond.col1 = ca.ancestor_concept_id
join concept c on cond.col1 = c.concept_id
left join condition_occurrence co on co.person_id = coh.person_id 
and co.condition_start_datetime < coh.visit_end_datetime
and co.condition_concept_id = ca.descendant_concept_id
group by coh.person_id, cond.col1, c.concept_name
)
-- QUERY
select condition_concept_id, condition_concept_name, sum(condition_present) as patient_count 
from cond
group by condition_concept_id, condition_concept_name