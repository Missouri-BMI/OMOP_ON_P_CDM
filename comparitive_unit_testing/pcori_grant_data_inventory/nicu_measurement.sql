...
-- Measurements during or before the NICU admission
meas as
(select coh.person_id, meas.col1 as measurement_concept_id, c.concept_name as measurement_concept_name, c.concept_code as measurement_concept_code,
sum(case when m.measurement_concept_id is null then 0 else 1 end) as total_measurements
from cohort coh
join (values
3015632, -- Carbon dioxide, total [Moles/volume] in Serum or Plasma
1175364, -- Neutrophils.immature/Neutrophils.total [Pure number fraction] in Blood - no records
3000905, -- Leukocytes [#/volume] in Blood by Automated count
3000963, -- Hemoglobin [Mass/volume] in Blood
3004249, -- Platelets [#/volume] in Blood by Automated count
3038687, -- Glucose [Mass/volume] in Serum or Plasma --6 minutes post XXX challenge
--3004501, -- glucose in serum or plasma - no records
--3038687, -- glucose - no records
--4149519,  -- glucose measurement - no records
--3010300, -- glucose in serum or plasma - no records
--3025673, -- glucose in serum or plasma - no records
3012888, -- Diastolic blood pressure
3016723, -- Creatinine [Mass/volume] in Serum or Plasma
3024171, -- respiratory rate - no records
--4313591, -- respiratory rate - no records
3024929, -- Systolic blood pressure
3027018, -- Heart rate - no records
3027598, -- Mean blood pressure - no records
3009343, -- pH of Capillary blood
3020891,-- Body temperature
36716470, -- Temperature
3025315, -- Body weight
--3013762, -- weigth
--3023166, -- weigth
3029943 -- Horowitz index in Arterial blood - no records
) as meas
join concept c on meas.col1 = c.concept_id
left join measurement m on m.person_id = coh.person_id 
and m.measurement_datetime < coh.visit_end_datetime
and m.measurement_concept_id = c.concept_id
group by coh.person_id, meas.col1, c.concept_name, c.concept_code
)

-- QUERY
select measurement_concept_id, measurement_concept_name,  measurement_concept_code,
min(total_measurements) as min_measurement_per_person, max(total_measurements) as max_measurement_per_person, round(avg(total_measurements)) as avg_measurement_per_person
from meas
group by measurement_concept_id, measurement_concept_name,  measurement_concept_code
