...
/**********************************************/
DROP TABLE IF EXISTS dj6i7sc8tmp_counts;

CREATE TEMP TABLE dj6i7sc8tmp_counts
AS
WITH counts  AS (
...
DROP TABLE IF EXISTS dj6i7sc8tmp_counts_person;

CREATE TEMP TABLE dj6i7sc8tmp_counts_person
AS
...
DROP TABLE IF EXISTS dj6i7sc8tmp_concepts;

CREATE TEMP TABLE dj6i7sc8tmp_concepts
AS
WITH concepts  AS (
  select concept_id as ancestor_id, coalesce(cast(ca.descendant_concept_id as varchar(50)), concept_id) as descendant_id
  from (
    select concept_id from dj6i7sc8tmp_counts
    UNION
    ...
    from dj6i7sc8tmp_counts c
    join vocab.concept_ancestor ca on cast(ca.descendant_concept_id as varchar(50)) = c.concept_id
  ) c
  left join vocab.concept_ancestor ca on c.concept_id = cast(ca.ancestor_concept_id as varchar(50))
)
 SELECT
ancestor_id,
  descendant_id
...
FROM dj6i7sc8tmp_concepts concepts
         LEFT JOIN dj6i7sc8tmp_counts c1 ON concepts.ancestor_id = c1.concept_id
         LEFT JOIN dj6i7sc8tmp_counts c2 ON concepts.descendant_id = c2.concept_id
         LEFT JOIN dj6i7sc8tmp_counts_person c3 ON concepts.ancestor_id = c3.concept_id
         LEFT JOIN dj6i7sc8tmp_counts_person c4 ON concepts.descendant_id = c4.concept_id
