{% if site in ['mu', 'mu-id'] %}
  {% set person_id_expr = "demographic.patid" %}
{% elif site == 'gpc' %}
  {% set person_id_expr = "demographic.patient_num" %}
{% else %}
  {% set person_id_expr = "demographic.patid" %}
{% endif %}
...
SELECT
    {{ person_id_expr }}::INTEGER                                   AS person_id,
    ...
    LEFT(COALESCE(demographic.raw_sex, ''), 50)::VARCHAR(50)         AS gender_source_value,
    0::INTEGER                                                      AS gender_source_concept_id,
    LEFT(COALESCE(demographic.raw_race, ''), 50)::VARCHAR(50)        AS race_source_value,
    0::INTEGER                                                      AS race_source_concept_id,
    LEFT(COALESCE(demographic.raw_hispanic, ''), 50)::VARCHAR(50)    AS ethnicity_source_value,
