...
SELECT DISTINCT
    ...
    , COALESCE(lab.lab_loinc, '')::VARCHAR(50) AS measurement_source_value
    , 0::INTEGER AS measurement_source_concept_id
    , COALESCE(lab.result_unit, '')::VARCHAR(50) AS unit_source_value
    , NULL::INTEGER AS unit_source_concept_id
    , COALESCE(
        LEFT(COALESCE(lab.raw_result, ''), 50),
        CASE WHEN lab.result_num = 0 THEN COALESCE(lab.result_qual, '') ELSE NULL END
