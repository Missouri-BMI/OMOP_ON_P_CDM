...
/**********************************************/
DROP TABLE IF EXISTS t4gnl28stmp_counts;

CREATE TEMP TABLE t4gnl28stmp_counts
AS
WITH counts  AS (
...
DROP TABLE IF EXISTS t4gnl28stmp_counts_person;

CREATE TEMP TABLE t4gnl28stmp_counts_person
AS
...
DROP TABLE IF EXISTS t4gnl28stmp_concepts;

CREATE TEMP TABLE t4gnl28stmp_concepts
AS
WITH concepts  AS (
  select concept_id as ancestor_id, coalesce(cast(ca.descendant_concept_id as varchar(50)), concept_id) as descendant_id
  from (
    select concept_id from t4gnl28stmp_counts
    UNION
...
FROM t4gnl28stmp_concepts concepts
         LEFT JOIN t4gnl28stmp_counts c1 ON concepts.ancestor_id = c1.concept_id
         LEFT JOIN t4gnl28stmp_counts c2 ON concepts.descendant_id = c2.concept_id
         LEFT JOIN t4gnl28stmp_counts_person c3 ON concepts.ancestor_id = c3.concept_id
         LEFT JOIN t4gnl28stmp_counts_person c4 ON concepts.descendant_id = c4.concept_id
